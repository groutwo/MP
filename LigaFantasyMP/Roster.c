...
void listPlayersByRoster( int rosterID ){
    ...
    char str[ 64 ];
    printf( "\nLista de jugadores:\n" );
    for( i = 0 ; i < tamPlayers ; i++){
        if( rPlayers[i].rosterID == rosterID ){
            sprintf( str, "%d \n", rPlayers[i].playerID ); //[TO-DO]: Get player info from Player struct.
            printf( "%s\n", str );
        }
    }
    printf( "\n" );
...
void deletePlayerFromRoster( int rosterID ){
    ...
    for( i=0 ; i < tamPlayers ; i++ ){
        if( rPlayers[i].playerID == cod && rPlayers[i].rosterID == rosterID ) {
            aux = 1;
            i++;
        }
        if( aux == 1 )
            rPlayers[i-1] = rPlayers[i];
    }
    if(aux == 1)
        rPlayers = ( T_RosterPlayers * ) realloc( rPlayers, ( --tamPlayers )*sizeof( T_RosterPlayers ));
}
...
void loadRostersPlayers(){
	...
	if(( file = fopen( "Jugadores_Plantillas.txt" , "r" )) == NULL ){
        ...
        int i;
        for( i=0 ; i<tamPlayers ; i++){
            printf("%d - %d\n", rPlayers[i].playerID, rPlayers[i].rosterID);
        }
    }
}
...
void saveRostersPlayers(){
    ...
    if(( file = fopen( "Jugadores_Plantillas.txt" , "w+" )) == NULL){
        ...
        for( i = 0 ; i < tamPlayers ; i++ ){
            sprintf( str, "%02d-%03d", rPlayers[i].playerID, rosters[i].rosterID );
            if( i != tamPlayers - 1) //Fix the \n on the last line.
                strcat(str, "\n");
            fputs( str, file );
        }
        fclose(file);
...
void addPlayerToRoster( int userID, int rosterID ){
    ...
    if( getNumPlayersInRoster( rosterID ) >= getMaxPlayersPerTeam() )
        ...
        if( rosters[getIdx(userID, rosterID)].rosterMoney < player.price )
            ...
            rosters[getIdx(userID, rosterID)].rosterMoney -= player.price; //Substract price of player to rooster

            rPlayers = (T_RosterPlayers *) realloc(rPlayers, ( tamPlayers + 1  )* sizeof( T_RosterPlayers ));

            rPlayers[tamPlayers].playerID = playerID;
            rPlayers[tamPlayers].rosterID = rosterID;
...
int chooseRoster( int userID ){
    ...
    listRostersByUserID( userID );
    do{
        printf("Introduce el ID de la platilla desea configurar: \n");
        scanf("%d", &rosterID);
    }while( rosters[getIdx(userID, rosterID)].userID != userID );
